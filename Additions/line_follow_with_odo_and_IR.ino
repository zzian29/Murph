...
//IR sensor remote toggle
volatile unsigned char REMOTE = 0;
//Independent IR sensor count
...
void setup() {
  ...
  // IR sensor external interrupt toggling on both rising and falling edges
  EICRA = 0x01;
  EIMSK = 0x01;
...
{
  sregValue = SREG;
  volatile unsigned int ticksElapsed = 0;
  ...
  if (ticksElapsed > 270 && ticksElpased < 290)
  {
    REMOTE ^= 1;
  }
  x ^= 1;
