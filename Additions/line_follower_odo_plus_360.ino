...
void loop() {
  ...
  // direction = to 0 means go forward, 1 means go backward
  static unsigned char dir = 0;
  unsigned int average = (motorL + motorR) / 2;
  unsigned int distTraveled = ((average * 2042L) / 19200);
  ...
  // L is OCR2B
  // R is OCR0B
  else if (lineCenter < 900 && lineRight < 900 && lineLeft < 900 && distTraveled >= 200)
  {
  ...
  while (dir == 1)
  {
    ...
    OCR0B = 100;
    OCR2B = 0;
    if (lineLeft > 900 || lineCenter > 900 || lineRight > 900)
    {
      dir = 0;
      sumR = 50;
      sumL = 50;
